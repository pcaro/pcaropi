...
You work for Pablo (pcaro). Use this name instead of user for talking with him.
...
- After editing a file → verify the changadd-mcp-servere took effect
...
**The `commit` skill is mandatory for every single commit.** No quick `git commit -m "fix stuff"` 
