			...
			try {
				...
				if (entry.type === "session") {
					...
					if (msg.usage && msg.provider && msg.model) {
						...
						seenHashes.add(hash);
						// Check if cost is valid (positive finite number)
						const rawCost = msg.usage.cost?.total;
						const cost = typeof rawCost === "number" && rawCost > 0 && Number.isFinite(rawCost) ? rawCost : 0;
						const invalidCost = cost === 0 && typeof rawCost === "number";
...
function formatCost(cost: number): string {
	// Handle edge cases: negative costs, NaN, non-numeric values
...
class UsageComponent {
	...
	render(width: number): string[] {
		const allLines = [...this.renderTitle(),
			...this.renderTabs(),
			...this.renderHeader(),
			...this.renderRows(),
			...this.renderTotals(),
	...
	private renderDataRow(
		...
		for (const col of DATA_COLUMNS) {
			const value = truncateToWidth(col.getValue(stats), col.width);
	...
	private renderTotals(): string[] {
		...
		for (const col of DATA_COLUMNS) {
			const value = truncateToWidth(col.getValue(stats.totals), col.width);
	...
	private renderFooter(): string[] {
		...
		// Help text
		lines.push(th.fg("dim", "[Tab/←→] period  [↑↓] select  [Enter] expand  [q] close"));

		// Warning about cost data if there are errors
		if (stats.modelsWithErrors.size > 0) {
			const errorCount = stats.modelsWithErrors.size;
			lines.push(th.fg("warning", `[!] Cost data incomplete: ${errorCount} model(s) had invalid cost data`));
		}

		// Note about approximation
		lines.push(th.fg("dim", "[i] Cost is approximate; depends on provider reporting"));
